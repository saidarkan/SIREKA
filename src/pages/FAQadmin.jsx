...
export default function Faq() {
  ...
  const handleUpdate = async (e) => {
    ...
    try {
      ...
      setError("");
      setSuccess("");
      await FaqAPI.update(editId, formData);
    ...
    <div className="w-full p-6">
      ...
      <div className="bg-white rounded-2xl shadow-lg overflow-hidden mt-10">
        ...
        {!loading && faqs.length > 0 && (
          <GenericTable
            columns={["No", "Pertanyaan", "Jawaban", "Aksi"]}
            data={faqs}
            renderRow={(item, index) => [
              <td key="no" className="px-6 py-4 text-center">
                {index + 1}
              </td>,
              <td key="pertanyaan" className="px-6 py-4 text-center">
                {item.pertanyaan}
              </td>,
              <td
                key="jawaban"
                className="px-6 py-4 max-w-xs break-words "
              >
                {item.jawaban}
              </td>,
